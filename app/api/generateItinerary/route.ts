...
const activitySchema: Schema = {
  ...
  properties: {
    ...
    type: { type: Type.STRING, enum: ['activity', 'meal', 'transit', 'rest'] },
    location: { type: Type.STRING, description: "Location name or address" },
    ecoTip: { type: Type.STRING, description: "A specific eco-friendly tip for this activity (e.g. bring reusable bottle, respect silence)" },
    durationHint: { type: Type.STRING, description: "Estimated duration (e.g. 2 hours)" }
  },
...
const daySchema: Schema = {
  ...
  properties: {
    ...
    theme: { type: Type.STRING, description: "Main theme of the day (e.g., Historic Center)" },
...
const itinerarySchema: Schema = {
  ...
  properties: {
    ...
    summary: { type: Type.STRING, description: "A 2-sentence summary of the trip vibe" },
    ...
    seasonalEvents: {
      type: Type.ARRAY,
      items: { type: Type.STRING },
      description: "List of 1-2 generic seasonal highlights typical for this destination."
    }
  },
  required: ["title", "summary", "days", "localEtiquette"]
};

export const generateItinerary = async (params: SearchParams): Promise<Itinerary> => {
  if (!apiKey) {
